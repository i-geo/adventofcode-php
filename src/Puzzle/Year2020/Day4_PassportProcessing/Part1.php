...
class Part1 implements PuzzleInterface
{
     ...
     * @return array
     */
    private function extractPassports(mixed $input): array
    {
        if (empty($input)) {
            return [];
        }
        ...
        foreach ($data as $i => $datum) {
            $passportIndex = $i + 1;
            try {
                $passportRaw = $this->extractPassport($datum);
                $passport = new Passport($passportRaw, $validator);
                echo "Passport $passportIndex OK: " . $passport . PHP_EOL;
                $prepared[] = $passport;
            } catch (InvalidInput | InvalidPassport $e) {
                echo "Passport $passportIndex invalid: {$e->getMessage()}." . PHP_EOL;
                continue;
            }
        }
    ...
    /**
     * Extracts passport data from give format
     * @param string $text
     * @return array
     * @throws InvalidInput
     */
    ...
    {
        $fieldsRaw = explode(self::FIELD_SEPARATOR, $text);
        $extracted = [];
        foreach ($fieldsRaw as $fieldRaw) {
            $fieldData = explode(self::VALUE_SEPARATOR, $fieldRaw);
            if (empty($fieldData) || (count($fieldData) !== 2)) {
                throw new InvalidInput(sprintf("Invalid field value separator '%s'", $fieldRaw));
            }
            if ($fieldData[0] === '') {
                throw new InvalidInput(sprintf("Empty field name '%s'", $fieldRaw));
            }
            if ($fieldData[1] === '') {
                throw new InvalidInput(sprintf("Empty field value '%s'", $fieldRaw));
            }

            $extracted[$fieldData[0]] = $fieldData[1];
        }
        return $extracted;
    }
